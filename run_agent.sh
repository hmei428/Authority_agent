...
set -euo pipefail
cd /mnt/tidal-alsh01/usr/heiwei/meihaojie/agent/Authority_agent-main
sudo chmod -R 777 /mnt/tidal-alsh01/usr/heiwei/meihaojie/agent/Authority_agent-main
# ========================================
# 配置区域
# ========================================
...
export INPUT_OSS_BUCKET="${INPUT_OSS_BUCKET:-xhs-bigdata-bravo}"
...
OUTPUT_DIR="${OUTPUT_DIR:-/mnt/tidal-alsh01/usr/heiwei/meihaojie/agent/Authority_agent-main/outputs/${DATE}}"
#选择输入要不要从oss中读取
DOWNLOAD_INPUT_FROM_OSS="${DOWNLOAD_INPUT_FROM_OSS:-true}"
DOWNLOAD_INPUT_USE_DATE="${DOWNLOAD_INPUT_USE_DATE:-false}"
OSS_INPUT_PREFIX="${OSS_INPUT_PREFIX:-oss://xhs-bigdata-bravo/user/search/retrieve/websearch/temp/meihaojie_websearch_input_query_agent}"
LOCAL_OSS_INPUT_DIR="${LOCAL_OSS_INPUT_DIR:-/mnt/tidal-alsh01/usr/heiwei/meihaojie/agent/Authority_agent-main/input/oss_${DATE}}"
...
# Checkpoint配置
CHECKPOINT_INTERVAL="${CHECKPOINT_INTERVAL:-1000}"  # 默认0表示禁用，可设置为50、100等
